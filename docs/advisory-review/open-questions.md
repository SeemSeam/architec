...
- plan/diff consistency 的路径级输入来源已由 [037-plan-diff-consistency-v1.md](decisions/037-plan-diff-consistency-v1.md) 决策为读取完整 saved plan-review JSON；structured dependency import expectations 已由 [040-plan-diff-import-edge-expectations.md](decisions/040-plan-diff-import-edge-expectations.md) 收敛。更深的 expected tests、public API migration notes 或 dependency alternatives 仍未决策。
