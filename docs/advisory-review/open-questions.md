...
- Hippocampus dogfood re-run after Decisions 041-047 confirmed diff/since scope hygiene but exposed remaining detector precision questions: whether `near_duplicate` should group intentional same-file variant families, whether `shadow_implementation` mapper role taxonomy should split color/rename/shape mapping responsibilities, and whether duplication `fix-advice` should recognize legacy/compat intent more explicitly.
