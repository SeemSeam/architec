...
- python-dotenv dogfood exposed full-review context calibration questions: active changelogs/release notes can be mistaken for stale docs, cleanup and archive can duplicate attention for the same path, and low-pressure topology findings can appear as boundary concerns even when `needs_folder_management=false`.
