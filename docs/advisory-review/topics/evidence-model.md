...
Advice feedback ledger 不是 ReviewConcern schema。按 [Decision
067](../decisions/067-advice-feedback-ledger.md)，reviewer feedback 是项目本地
输入，用于 full-review recommendations 和 `fix-advice` 降噪。Feedback entries
可以引用 `advice_id`、`concern_id`、path、kind 和 reviewer status，但不改变已有
concern 的 factual identity，也不从 artifacts 中删除原始 scanner output。
当 feedback 降级 advice 时，结果通过 `artifacts.advice_feedback` 记录。
...
When incremental code-review runs through the default `archi` path, a
`snapshot_context` signal may record Hippo bundle presence, selected-file
freshness relative to the snapshot, unknown freshness, refresh status, and
bounded stale reasons. This signal is context only; it does not create concerns
and does not make selected-scope review equivalent to a refreshed full-project
baseline.
...
`snapshot_context` signal 的 `metrics` 可包含 `hippo_bundle_present`、`hippo_bundle_stale`、`freshness_unknown`、`hippo_refresh_performed`、`missing_file_total`、`stale_reason_total`、`selected_file_total`、`selected_changed_after_bundle_total` 和 `stale_reasons`。这些 metrics 只说明默认增量审查所处的 Hippo snapshot 上下文，不是健康分，也不要求本次审查自动刷新 Hippo。
