...
Full review reliability v1 见 [Decision 058](../decisions/058-full-code-review-static-degradation.md)：当 full code-review 无法使用 Hippo bundle 或 backend LLM path 时，CLI 会返回明确标记的 static CodeReviewResult，而不是直接中断 dogfood。该结果使用 `summary.analysis_mode=static`、`summary.reason` 和 `artifacts.code_review_static_reason` 标记降级原因，并保留 deterministic `near_duplicate`、`shadow_implementation` 和 advisory discovery signals。

Incremental static degradation 见 [Decision 066](../decisions/066-incremental-static-degradation.md)：当 diff/since 的 backend LLM path 不可用但 git selected-file scope 仍可读取时，CLI 会返回明确标记的 static incremental CodeReviewResult。该结果只保留 selected-scope deterministic signals（near duplicate、shadow implementation、architecture contracts、plan/diff consistency 和可选 risk context），不运行 full-review global cleanup/archive/hotspot/topology context。since bad-ref 和 bundle validation 错误仍保持原有边界。

Incremental-first LLM cost control 见 [Decision 068](../decisions/068-incremental-first-cost-control.md)：公开入口收敛为 `archi` 增量 LLM 审查和 `archi --full` 全量 LLM 审查。增量路径仍然使用 LLM，但 prompt 只包含 selected-scope deterministic evidence、changed-file summary、相关 rules、plan/risk context，而不是 full-project context；成本控制来自 compact context、有界 LLM 调用、无默认 full Hippo refresh 和缓存复用。V1 已让裸 `archi` 路由到 selected-change incremental LLM review，并输出初版 `cost_context` telemetry；elapsed time 和真实 cache hit count 仍是后续打磨项。

Incremental snapshot freshness context 见 [Decision 069](../decisions/069-incremental-snapshot-freshness-context.md)：默认 `archi` 不刷新 Hippo，但会输出 `snapshot_context` signal，说明 Hippo bundle 是否存在、freshness 是否可判断、selected files 是否晚于 snapshot、是否发生 refresh，以及有限的 stale reasons。该上下文也进入 selected-scope LLM payload，帮助 LLM 不把增量结果误写成 refreshed full-project baseline。
...
Full review 可以通过 `--advice-feedback <json>` 读取项目本地 reviewer
feedback，避免同一条已被判定错误或不适用的 recommendation 反复进入默认
human-facing summary。Ledger 只影响 recommendations / fix-advice 展示降噪；
完整 generated artifacts 仍保留原始 scanner 输出。
