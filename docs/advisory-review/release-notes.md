...
Full-review context calibration v1 is now display/top-concern scoped behavior: active changelog and release-note stale-doc observations are suppressed or demoted from default top concerns, cleanup/archive same-path concerns no longer both occupy top-level concern slots, and low-pressure topology findings stay signal context when `needs_folder_management=false` and flat file count is small. Raw signals and artifacts remain available.
