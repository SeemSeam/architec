...
def _compact_hotspot_item(item: dict[str, Any]) -> dict[str, Any]:
    return {
...
def compact_hotspots(hotspot_digest: dict[str, Any], limit: int = 8) -> list[dict[str, Any]]:
    ...
    for item in items[: max(1, limit)]:
        if not isinstance(item, dict):
            continue
        out.append(_compact_hotspot_item(item))
    return out
        ...
        out.append(
            {
                ...
                "focus_files": [
                    str(path)
                    for path in (focus_files if isinstance(focus_files, list) else [])[:6]
                ],
                "why": item.get("why", {}),
            }
        )
...
def _descriptor_neighbors(descriptor: dict[str, Any]) -> list[str]:
    return [
...
def _descriptor_tests(descriptor: dict[str, Any]) -> list[str]:
    return [str(path or "") for path in descriptor.get("test_anchors", [])[:4]]
...
def _compact_descriptor_item(component: str, descriptor: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "dependency_neighbors": _descriptor_neighbors(descriptor),
        "test_anchors": _descriptor_tests(descriptor),
    }
