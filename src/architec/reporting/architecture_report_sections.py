...
def _component_context_lines(item: dict[str, Any]) -> list[str]:
    component = str(item.get("component", "") or "")
    layer_role = str(item.get("layer_role", "") or "")
    confidence = float(item.get("confidence", 0.0) or 0.0)
    summary = str(item.get("responsibility_summary", "") or "")
    lines = [f"- `{component}` | layer=`{layer_role}` | confidence=`{confidence}` | {summary}"]
...
def _render_component_context(lines: list[str], descriptors: list[dict[str, Any]]) -> None:
    ...
    for item in descriptors[:8]:
        if not isinstance(item, dict):
            continue
        lines.extend(_component_context_lines(item))
    lines.append("")


def _render_llm_plan_item(item: dict[str, Any]) -> list[str]:
...
def _render_fallback_plan_item(item: dict[str, Any]) -> str:
    return (
    ...
    *,
    ...
    if llm_plan:
        for item in llm_plan[:12]:
            if not isinstance(item, dict):
                continue
            lines.extend(_render_llm_plan_item(item))
    else:
        for item in batches[:8]:
            lines.append(_render_fallback_plan_item(item))
    lines.append("")
...
def _governance_lines(gov: dict[str, Any]) -> list[str]:
    return [
        "## Governance Scores",
        f"- Full: `{gov.get('full', 0.0)}`",
        f"- Incremental: `{gov.get('incremental', 0.0)}`",
        f"- Overall: `{gov.get('overall', 0.0)}`",
        "",
    ]
...
def render_architecture_report_markdown(
    *,
    ...
    title, summary = _title_and_summary(llm_doc)
    ...
    lines.extend(_governance_lines(governance))
