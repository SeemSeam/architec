...
from .bundle_loader import inspect_bundle, require_bundle
from .resource_paths import resolve_config_file, tool_script_path
...
def refresh_bundle_from_hippo(project_root: str | Path) -> dict[str, Any]:
    ...
    steps = [
        [*base_cmd, "init", "--target", str(root)],
        [*base_cmd, "sig-extract", "--target", str(root)],
        [*base_cmd, "tree", "--target", str(root)],
        [*base_cmd, "index", "--target", str(root), "--no-llm"],
        [
            *base_cmd,
            "structure-prompt",
            "--target",
