...
def test_status_trend_returns_empty_trend_without_events(tmp_path) -> None:
    ...
    assert result["trend"]["summary"] == "No review events were recorded."
    assert result["trend"]["reason"] == "Status trend needs review events generated by code-review."
    assert result["weakening_components"] == []
    assert result["artifacts"]["review_event_jsonl"] == str(tmp_path / ".architec" / "review-events.jsonl")
    encoded = json.dumps(result["trend"], sort_keys=True).lower()
...
def test_status_scores_empty_without_full_event(tmp_path) -> None:
    ...
    assert result["trend"]["summary"] == "No full code-review events were recorded for status scores."
    assert result["trend"]["reason"] == "Status scores use the latest full code-review event."
    encoded = json.dumps(result["trend"], sort_keys=True).lower()
    for term in ("pass", "fail", "block", "verdict", "must-fix", "clean", "safe"):
        assert term not in encoded
